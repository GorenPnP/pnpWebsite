...
class KlasseStufenplan(models.Model):
    ...
    def get_choosable_KlasseStufenplan(char: "Charakter") -> list["KlasseStufenplan"]:
        ...
        astrale_reaktion = attrs["MA"] + attrs["WK"] + char.astrale_reaktion_bonus
        if char.no_MA_MG:
            astrale_reaktion += 4
        elif char.no_MA:
            astrale_reaktion = attrs["WK"] + char.astrale_reaktion_bonus
...
class Gfs(models.Model):
    ...
    eigenschaften = models.ManyToManyField(GfsEigenschaft, blank=True)
...
class Charakter(models.Model):
    ...
    # Kampagne
    ep = models.PositiveIntegerField(default=0)
    ep_stufe = models.PositiveIntegerField(default=1, verbose_name="aktuelle Stufe des Charakters")
    ep_stufe_in_progress = models.PositiveIntegerField(default=1, verbose_name="Stufe des Charakters, die noch verteilt werden muss")
    ...
    astrale_reaktion_bonus = models.SmallIntegerField(default=0)
...
class GfsSkilltreeEntry(models.Model):
    ...
    def __repr__(self) -> str:
        try:
            ...
            # Initiative fix, physische & astrale Reaktion, physischer Widerstand
    ...
    def apply_to(self, char: Charakter) -> None:
        ...
        # astrale Reaktion
        if self.operation == "T":
            char.astrale_reaktion_bonus += self.amount
            char.save(update_fields=["astrale_reaktion_bonus"])
            return
