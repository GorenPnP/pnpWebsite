...
def character_export(request: HttpRequest, pk, *args, **kwargs) -> HttpResponse:
...
def generate_char_xslx(char: Charakter):
    ...
    werte_ws.merge_range("B1:D1", char.name if char.name else "<NO NAME>")
    ...
    werte_ws.merge_range("J34:L34", char.beruf.titel if char.beruf else "", format_person)
    ...
    werte_ws.merge_range("J36:L36", char.religion.titel if char.religion else "", format_person)
    ...
    # Gfs-Fähigkeiten
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Wesen-Eigenschaften/Fähigkeiten", format_ramsch_titel)
    if char.gfs:
        for r in RelGfsAbility.objects.filter(char=char):
            ROW_NUMBER += 1
            werte_ws.write(f"A{ROW_NUMBER}", r.ability.name, format_border_left)
            werte_ws.merge_range(f"B{ROW_NUMBER}:G{ROW_NUMBER}", r.ability.beschreibung, format_text_wrap)
            werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    # Zauber, Rituale & Runen
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Zauber, Rituale und Runen", format_ramsch_titel)
    for r in char.relzauber_set.all():
        ROW_NUMBER += 1
        werte_ws.write_row(f"A{ROW_NUMBER}", [r.item.name], format_border_left)
        werte_ws.write_row(f"B{ROW_NUMBER}", [r.tier])
        werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    for r in char.relrituale_runen_set.all():
        ROW_NUMBER += 1
        werte_ws.write_row(f"D{ROW_NUMBER}", [r.anz, r.item.name, f"Stufe {r.stufe}"])
        werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    # Talente
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Talente", format_ramsch_titel)
    for r in char.reltalent_set.all():
        ROW_NUMBER += 1
        werte_ws.write_row(f"A{ROW_NUMBER}", [r.talent.titel], format_border_left)
        werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    # Wesenkraft
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Wesenkräfte", format_ramsch_titel)
    for r in char.relwesenkraft_set.all():
        ROW_NUMBER += 1
        werte_ws.write_row(f"A{ROW_NUMBER}", [r.wesenkraft.titel], format_border_left)
        werte_ws.write_row(f"B{ROW_NUMBER}", [r.tier])
        werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    # Items
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Items", format_ramsch_titel)
    for r in char.relitem_set.all():
        ROW_NUMBER += 1
        werte_ws.write(f"A{ROW_NUMBER}", r.anz, format_border_left)
        werte_ws.write(f"B{ROW_NUMBER}", r.item.name)
        werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    # Waffen & Werkzeuge
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Waffen & Werkzeuge", format_ramsch_titel)
    for r in char.relwaffen_werkzeuge_set.all():
        ROW_NUMBER += 1
        werte_ws.write(f"A{ROW_NUMBER}", r.anz, format_border_left)
        werte_ws.write(f"B{ROW_NUMBER}", r.item.name)
        werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    # Schusswaffen
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Schusswaffen", format_ramsch_titel)
    for r in char.relschusswaffen_set.all():
        ROW_NUMBER += 1
        werte_ws.write(f"A{ROW_NUMBER}", r.anz, format_border_left)
        werte_ws.write_row(f"B{ROW_NUMBER}", [r.item.name, f"BS {r.item.bs} ({r.item.erfolge} Erfolge)", f"ZS {r.item.zs}", f"Präzi {r.item.präzision}"])
        werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    # Ausrüstung & Technik
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Ausrüstung und Technische Geräte", format_ramsch_titel)
    for r in char.relausrüstung_technik_set.all():
        ROW_NUMBER += 1
        werte_ws.write(f"A{ROW_NUMBER}", r.anz, format_border_left)
        werte_ws.write(f"B{ROW_NUMBER}", r.item.name)
        werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    # TODO ?
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Holoboard & Zubehör", format_ramsch_titel)
    # Einbauten
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Cyber- und Bioware", format_ramsch_titel)
    for r in char.releinbauten_set.all():
        ROW_NUMBER += 1
        werte_ws.write(f"A{ROW_NUMBER}", r.anz, format_border_left)
        werte_ws.write(f"B{ROW_NUMBER}", r.item.name)
        werte_ws.write(f"H{ROW_NUMBER}", None, format_border_right)
    ...
    # Affektivität
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Personen, Begleiter und Haustiere", format_ramsch_titel)
    ROW_NUMBER += 1
    werte_ws.write_row(f"A{ROW_NUMBER}", ["Name", "Wert", "Grad"], format_ramsch_titel)
    werte_ws.merge_range(f"D{ROW_NUMBER}:E{ROW_NUMBER}", "UM", format_ramsch_titel)
    werte_ws.merge_range(f"F{ROW_NUMBER}:H{ROW_NUMBER}", "Notizen", format_ramsch_titel)
    personen_aff = char.affektivität_set.values_list("name", "wert", Value("?"), "notizen")
    for aff in personen_aff:
        ROW_NUMBER += 1
        werte_ws.write(f"A{ROW_NUMBER}", aff[0] if aff is not None else None, format_border_left_right)
        werte_ws.write_row(f"B{ROW_NUMBER}", aff[1:3] if aff is not None else [None, None], format_border_right)
        werte_ws.merge_range(f"D{ROW_NUMBER}:E{ROW_NUMBER}", "?", format_border_right)
        werte_ws.merge_range(f"F{ROW_NUMBER}:H{ROW_NUMBER}", aff[3] if aff is not None else None, format_border_right)
    # Notizen
    notizen = [char.sonstige_items, char.persönlicheZiele, char.notizen]
    ...
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER}", "Notizen", format_ramsch_titel)
    ROW_NUMBER += 1
    werte_ws.merge_range(f"A{ROW_NUMBER}:H{ROW_NUMBER+4}", "\n".join([n for n in notizen if n]), format_large_cell)
    ROW_NUMBER += 4
