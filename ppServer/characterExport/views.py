...
class CharacterExportView(LoginRequiredMixin, VerifiedAccountMixin, DetailView):
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        # center down
        format_gsh_titel = wb.add_format(form_sub_titel)
        ...
        format_limit_topic = wb.add_format(dict(form_section_titel, **{"bg_color": COLOR["grau 1"]}))
        format_limit_titel = wb.add_format(dict(form_sub_titel, **{"bg_color": COLOR["grau 1"]}))
        ...
        werte_ws.write_row(4, 8, ["Initiative", "=N9+N5"], format_ini)
        werte_ws.write_row(5, 8, ["W4", "=N3"], format_ini)
        werte_ws.merge_range("I7:I8", "Astral-WI", format_asWi_topic)
        werte_ws.merge_range("J7:J8", "=N9+N10", format_asWi)
        werte_ws.write_row("I9", ["Limits", ""], format_limit_topic)
        ...
        format_state_titel = wb.add_format(dict(form_bold, **form_italic, **form_align_center_center))
        format_state_subtitel = wb.add_format(dict(form_italic, **form_align_center_center))
        ...
        format_state_percent__last = wb.add_format(dict(form_align_center_center, **{"bottom": 1}))
        ...
        werte_ws.write("I21", "<80%", format_align_center_center)
        werte_ws.write("J21", -1, format_state)
        werte_ws.write("I22", "<60%", format_align_center_center)
        werte_ws.write("J22", -2, format_state)
        werte_ws.write("I23", "<50%", format_align_center_center)
        werte_ws.write("J23", -3, format_state)
        werte_ws.write("I24", "<30%", format_align_center_center)
        ...
        format_wesen_titel = wb.add_format(dict(form_section_titel, **{"font_size": 8, "top": 1, "bottom": 1, "bg_color": COLOR["grau 3"]}))
        format_wesen = wb.add_format(dict(form_bold, **form_italic, **{"top": 1, "bottom": 1, "bg_color": COLOR["grau 3"]}))
        ...
        werte_ws.write("I29", "Gewicht:", format_align_center_center)
        werte_ws.merge_range("J29:L29", f"{char.gewicht} kg", format_person)
        werte_ws.write("I30", "Größe:", format_align_center_center)
        werte_ws.merge_range("J30:L30", f"{char.größe} cm", format_person)
        werte_ws.write("I31", "Alter:", format_align_center_center)
        werte_ws.merge_range("J31:L31", f"{char.alter}", format_person)
        werte_ws.write("I32", "Geschlecht:", format_align_center_center)
        werte_ws.merge_range("J32:L32", char.geschlecht, format_person)
        werte_ws.write("I33", "Sexualität:", format_align_center_center)
        werte_ws.merge_range("J33:L33", char.sexualität, format_person)
        werte_ws.write("I34", "Beruf:", format_align_center_center)
        werte_ws.merge_range("J34:L34", char.beruf.titel, format_person)
        werte_ws.write("I35", "Präf. Arm:", format_align_center_center)
        werte_ws.merge_range("J35:L35", char.präf_arm, format_person)
        werte_ws.write("I36", "Religion:", format_align_center_center)
        werte_ws.merge_range("J36:L36", char.religion.titel, format_person)
        werte_ws.write("I37", "Hautfarbe:", format_align_center_center)
        werte_ws.merge_range("J37:L37", char.hautfarbe, format_person)
        werte_ws.write("I38", "Haarfarbe:", format_align_center_center)
        werte_ws.merge_range("J38:L38", char.haarfarbe, format_person)
        werte_ws.write("I39", "Augenfarbe:", format_align_center_center)
        ...
        werte_ws.merge_range("M20:O20", "Rüstung Schutz (Stärke:    x     HP)", format_colorful_titel)
        werte_ws.merge_range("P20:R20", "x", format_colorful)
        werte_ws.write("S20", "s. Rüstung")
        werte_ws.merge_range("M21:O21", "Haltbarkeit der Rüstung", format_colorful_titel)
        werte_ws.merge_range("P21:R21", "x", format_colorful)
        werte_ws.write("S21", "s. Rüstung")
        werte_ws.merge_range("M22:O22", "Natürlicher Schadenswiderstand", format_colorful_titel_emph)
        ...
        werte_ws.merge_range("M25:O25", "Bewegung Astral (Meter pro 10 Sek.)", format_colorful_titel)
        werte_ws.merge_range("P25:R25", "=2*N10*(N9+N3)", format_colorful)
        werte_ws.write("S25", "2*MA*(WK+SCH)")
        werte_ws.merge_range("M26:O26", "Schwimmen (Meter pro 10 Sek.)", format_colorful_titel)
        werte_ws.merge_range("P26:R26", "=P24/3+G4/5", format_colorful)
        werte_ws.write("S26", "Gehrate/3+Schwimmen/5")
        werte_ws.merge_range("M27:O27", "Tauchen (Meter pro 10 Sek.)", format_colorful_titel)
        ...
        format_fert_attr = wb.add_format(dict(form_align_bottom_center, **{"font_size": 16}))
        format_fert_attr__odd = wb.add_format(dict(form_align_bottom_center, **{"font_size": 16, "bg_color": COLOR["grau 1"]}))
        format_fert_ap = wb.add_format(dict(form_align_bottom_center, **form_bold, **form_italic, **{"font_size": 18, "bg_color": COLOR["grau 1"]}))
        format_fg = wb.add_format(dict(form_align_bottom_center, **{"font_size": 14, "bg_color": COLOR["grau 1"]}))
        ...
        format_fert_titel = wb.add_format(dict(form_align_center_center, **{"font_size": 8, "left": 1}))
        format_fert_fp = wb.add_format(dict(form_align_center_center, **{"font_size": 12, "bg_color": COLOR["lavender"]}))
        format_fert_limit = wb.add_format(dict(form_align_center_center, **form_bold, **form_italic, **{"font_size": 12, "right": 1}))
        ...
        for fert in char.relfertigkeit_set.values("fertigkeit__titel", "fertigkeit__limit", "fp", "fp_bonus", attr1=F("fertigkeit__attr1__titel"), attr2=F("fertigkeit__attr2__titel")):
            ...
            werte_ws.write(f"{'E' if not fert['attr2']  else 'F'}{row}", fert["fp_bonus"], format_align_center_center)
            werte_ws.write(f"H{row}", fert["fertigkeit__limit"], format_fert_limit)

            if fert['attr2']:
                werte_ws.write(f"B{row}", f"{fert['attr1']} + {fert['attr2']}", format_fert_attr2)
                werte_ws.write(f"C{row}", f"={POSITION[fert['attr1']]}", format_fert_attr2)
                werte_ws.write(f"D{row}", f"={POSITION[fert['attr2']]}", format_fert_attr2)
        ...
        for i, a in enumerate(char.affektivität_set.values_list("name", "wert", Value("?"), Value("?"), "notizen")[:11]):
            index = 124+i
            werte_ws.write_row(f"A{index}", a[:3], format_border_right)
            werte_ws.merge_range(f"D{index}:E{index}", a[3], format_border_right)
            werte_ws.merge_range(f"F{index}:H{index}", a[4], format_border_right)
        # Notizen
        werte_ws.merge_range("A135:H135", "Notizen", format_ramsch_titel)
        werte_ws.merge_range("A136:H143", char.notizen, format_large_cell)

        werte_ws.write_row("A144", [None, None, None, None, None, None, None, None], format_border_top)
        for i in range(44, 144):
            werte_ws.write(f"I{i}", None, format_border_left)
        ...
        geld_ws.write("I1", "=Werte!N38", format_konz)
        geld_ws.merge_range("J1:K1", "Geistige HP Max.", format_section_titel)
        geld_ws.write("L1", "=Werte!N39", format_konz)
        ...
        geld_ws.merge_range("M17:M18", "=Werte!P33", format_fert_ap)
        ...
        geld_ws.write("A3", 15000000, format_fert_ap) # TODO ?
        ...
        werte_ws.set_column("K:O", 10)
        werte_ws.set_column("P:R", 5)
        
        spF_wF_ws.set_column("D:D", 10)
        spF_wF_ws.set_column("E:E", 15)
        spF_wF_ws.set_column("H:H", 13)
        spF_wF_ws.set_column("I:I", 10)
        spF_wF_ws.set_column("K:K", 13)
